export default function CounsellorStatsCards({ students, feedbacks }) {
  ...
  const highFeedbackRisk = feedbacks.filter((f) => f.risk_level === "High").length;
  const pendingAction = feedbacks.filter((f) => !f.action_taken).length;
  ...
  const cards = [
    ...
    { label: "High Stress Feedback", value: highFeedbackRisk, color: "text-orange-600", bg: "bg-orange-50 border-orange-100", icon: "😟" },
    { label: "Pending Action", value: pendingAction, color: "text-violet-600", bg: "bg-violet-50 border-violet-100", icon: "📋" },
  ];
